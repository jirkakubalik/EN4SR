...
def get_constraint_term(data, constr:Constraint, y_hat, weight=1.0):
    ...
    # --- Calculate loss
    loss_rc = tf.divide(tf.reduce_sum(tf.square(tf.math.maximum(tf.subtract(y_hat_r, y_hat_c), 0.))),
                              data.forwardpass_counts[labelBase+'_c'])
    loss_cl = tf.divide(tf.reduce_sum(tf.square(tf.math.maximum(tf.subtract(y_hat_c, y_hat_l), 0.))),
                                 data.forwardpass_counts[labelBase+'_c'])
    loss_cr_lc = tf.divide(tf.reduce_sum(tf.square(tf.math.maximum(tf.subtract(tf.subtract(y_hat_c, y_hat_r),tf.subtract(y_hat_l, y_hat_c)), 0.))),
                                 data.forwardpass_counts[labelBase+'_c'])
