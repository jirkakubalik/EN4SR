...
def get_constraint_term(data, constr:Constraint, y_hat, weight=1.0):
    ...
    eps = constr.args['eps']
    minDecrease = eps * np.tan(0.0000174533)      # --- min angle: 0.00174533 rad == 0.1 deg
    penalty = np.float64(0.01)
    # --- Calculate loss
    decrease = tf.math.maximum(tf.subtract(y_hatA, y_hatB), 0.)
    tf_penalty = tf.where(decrease < minDecrease, minDecrease, 0.0)     # --- penalty
    tf_error = tf.add(tf.math.maximum(tf.subtract(y_hatB, y_hatA), 0.), tf_penalty)
