# Output is equal to the specified state variable x_i
# f(x) == x_i
import numpy as np
import warnings
...
def get_constraint_term(data, constr:Constraint, y_hat, weight=1.0):
    ...
    # ---
    var_values = tf.multiply(data.forwardpass_data[:,varId].reshape((data.forwardpass_data.shape[0], 1)), data.forwardpass_masks[labelBase+'_x'])
    y_hatA = tf.multiply(y_hat, data.forwardpass_masks[labelBase+'_x'])
    # --- Calculate loss
    loss = weight * tf.divide(tf.reduce_sum(tf.square(tf.subtract(var_values, y_hatA))),
