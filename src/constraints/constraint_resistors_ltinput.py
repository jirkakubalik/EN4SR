...
def get_constraint_term(data, constr:Constraint, y_hat, weight=1.0):
    ...
    dataTest = tf.multiply(data.forwardpass_data, data.forwardpass_masks[labelBase+'_x'])
    # --- Calculate loss
    m = tf.reshape(tf.math.minimum(dataTest[:,0], dataTest[:,1]), (dataTest.shape[0], 1))
    loss = weight * tf.divide(tf.reduce_sum(tf.square(tf.math.maximum(tf.subtract(y_hatA, m), 0.))),
